...
var sqlInsert = async function(search){
    var sql = "insert into " + search.table +"(",sql_d = [],sql_temp = " values (";
    for(var condi in search.conditions){
        if(search.conditions[condi].length == 1){
            sql += "" + condi + ",";
            sql_temp += "?,";
    ...
    sql = sql.substring(0, sql.length-1) + ")" + sql_temp.substring(0, sql_temp.length-1)+")";
    return {sql:sql,sqlData:sql_d};
};

var connection = mysql.createConnection(dbconfigj);
async function query(sql, sql_d, callback) {
    var connection = mysql.createConnection(dbconfigj);
    connection.connect(async function (err) {
        ...
        connection.query(sql, sql_d, async function (err, results, fields) {
            ...
            console.log("1");
            await callback && await callback(results, fields);
            console.log("2");
